...
void check_constness() {
	...
	if (s.find(3) == s.end()) fail("3 not found, incorrect find");
	if (*s.lower_bound(2) != 3 || s.lower_bound(8) != s.end() || *s.lower_bound(-2) != 0) fail("incorrect lower_bound");
	if (s.empty()) fail("incorrect empty");
	if (s.size() != 5) fail("incorrect size");
	auto first = s.begin();
	auto last = s.end();
	if (*first != -4 || *(--last) != 7) fail("incorrect begin or end");
	std::cerr << "ok!\n";
}
...
void check_copy_correctness() {
	...
	while (s1_it != s1.end() || s2_it != s2.end() || s_it != set_elems.end()) {
		...
		if (*s1_it != *s2_it || *s1_it != *s_it || *s2_it != *s_it) fail("fail after copy construction and insertions");
		++s1_it, ++s2_it, ++s_it;
	}
	s1 = s2;
	s2.insert(19);
	...
	while (s1_it != s1.end() || s2_it != cur_end) {
		if (*s1_it != *s2_it) fail("wrong = operator");
		++s1_it, ++s2_it;
	}
...
void check_empty() {
	...
	Set<std::string> s;
	if (!s.empty()) fail("wrong empty");
	auto begin = s.begin(), end = s.end();
	if (begin != end) fail("incorrect handling empty set");
	std::string elem("abacaba");
	s.insert(elem);
	if (*s.lower_bound("aac") != elem) fail("wrong lower_bound");
	Set<std::string> empty;
	Set<std::string> s2{ "opa" };
	s2 = empty;
	if (s2.size()) fail("incorrect size");
	Set<std::string> s3(s2);
	if (!s3.empty()) fail("incorrect empty");
	std::cerr << "ok!\n";
}
...
void check_iterators() {
	...
	if (s.begin()->second != 1 || (++s.begin())->first != -4) fail("wrong begin()");
	auto cur = s.end();
	Set<int> small{ 1 };
	auto it = small.begin();
	if (*it != 1) fail("incorrect begin");
	auto begin = s.begin();
	begin++;
	cur--;
	if (begin == cur) fail("wrong == for iterators");
	while (begin != cur) {
		++begin;
		--cur;
	}
	if (*begin != *cur) fail("wrong Iterators");
	std::cerr << "ok!\n";
}
...
struct StrangeInt {
	...
	static int counter;
	StrangeInt() { ++counter; }
	StrangeInt(int _x) : x(_x) { ++counter; }
...
void check_operator_less() {
	...
	++it;
	if (it != s.end()) fail("wrong ++ for iterator");
	std::cerr << "ok!\n";
}
...
void check_destructor() {
	...
	{
		Set<StrangeInt> s{ 5 };
		if (s.size() != 1) fail("wrong size");
	}
	if (StrangeInt::counter) fail("wrong destructor (or constructors)");
	{
		Set<StrangeInt> s{ -3 };
		Set<StrangeInt> s1(s);
		s1.insert(0);
		Set<StrangeInt> s2(s1);
		if (s1.find(0) == s1.end()) fail("wrong find");
		s1 = s;
		if (s1.find(0) != s1.end()) fail("wrong find");
	}
	if (StrangeInt::counter) fail("wrong destructor (or constructors)");
...
void check_erase() {
	...
	s.erase("miss");
	s.erase("hello");
	if (s.size() != 2) fail("Bad size");
	s.erase("p");
	if (*s.begin() != "abacaba") fail("Bad begin");
	s.erase("1");
	s.erase("abacaba");
	s.erase("012");
	if (!s.empty()) fail("Bad empty");
	std::cerr << "ok!\n";
}
